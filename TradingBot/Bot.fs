...
type BotAction =
    | Wait
    | Sell of float * uint64
    | Buy of float * uint64
    with
        override x.ToString() =
            match x with
            | Wait -> "waiting"
            | Sell(r, a) -> sprintf "selling %.2f (%d)" (r * 100.0) a
            | Buy(r, a) -> sprintf "buying %.2f (%d)" (r * 100.0) a
...
let BuyPercentage (t : Timestamp) (rate : float) (perc : float) (prev : State) =
    ...
    {
        Risk = prev.Risk
        Budget = max 0.0 (prev.Budget - ((float buyvolume) * rate))
        ...
        LastAction = if buyvolume > 0UL then Buy(perc, buyvolume) else Wait
    }
...
let SellPercentage (t : Timestamp) (rate : float) (perc : float) (prev : State) =
                     ...
                     |> uint64
    {
        ...
        AssetCount = max 0UL (prev.AssetCount - sellvolume)
        TimestampBuy = prev.TimestampBuy
        TimestampSell = if sellvolume > 0UL then t else prev.TimestampSell
        ExchangeRate = Some rate
        LookBack = prev.LookBack
        LastAction = if sellvolume > 0UL then Sell(perc, sellvolume) else Wait
    }
...
let TradeHistory (bot : IBot) (rate : ExchangeRate) budget risk lookback =
    ...
    let sphist = splitwhen (fun (_, s) -> s.ExchangeRate.IsNone) hist

    Seq.toList (fst sphist)
    @ [snd sphist |> Seq.head]
    ...
    interface IBot with
        member __.Next t rate prev =
            ...
            | Some r0 -> let keep = update r0
                         if t < 2UL then                                keep prev // KEEP
                         else match frate(t - 1UL), frate(t - 2UL) with
                              | r1, r2 when r0 < r1             ->      if is_risky then SellPercentage t r0 1.0 prev // SELL 100%
                                                                                    else keep prev // KEEP
                              | r1, r2 when r0 = r1 && r1 >= r2 ->      if is_risky then BuyPercentage t r0 1.0 prev // BUY 100%
                                                                                    else BuyPercentage t r0 risk prev // BUY R%
                              | r1, r2 when r0 > r1 && r0 >= r2 ->      BuyPercentage t r0 1.0 prev // BUY 100%
                              | r1, r2 when r0 > r1             ->      BuyPercentage t r0 risk prev // BUY R%
                              | r1, r2 ->                               SellPercentage t r0 (if r0 > (frate prev.TimestampBuy) + abs(r0 - r1)
                                                                                             then irisk
                                                                                             else 1.0) prev // SELL
            | None -> match prev.ExchangeRate with
                      | Some r ->
                            let prev = if prev.AssetCount > 0UL
                                       then SellPercentage t (frate(t - 1UL)) 1.0 prev
                                       else prev
                            {
                                Risk = risk
                                Budget = prev.Budget
                                ...
                                LookBack = prev.LookBack
                                ExchangeRate = None
                                LastAction = Sell(1.0, prev.AssetCount)
                            }
                      | None -> prev
